...
public interface IInternalRepositoryController {
    ...
    ResponseEntity<RepositoryMetadata> get(@PathVariable Integer repositoryId) throws Exception;

    @RequestMapping(value = Routes.CREATE_INTERNAL_REPOSITORY_ROUTE, method = RequestMethod.POST)
    ResponseEntity<Integer> insert(@RequestBody RepositoryMetadata repository) throws Exception;

    @RequestMapping(value = Routes.UPDATE_INTERNAL_REPOSITORY_ROUTE, method = RequestMethod.PUT)
    ResponseEntity<Void> update(@PathVariable Integer repositoryId, @RequestBody RepositoryMetadata repository) throws Exception;
